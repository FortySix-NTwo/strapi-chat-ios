...
class RegistrationController: UIViewController {
  ...
  @objc func signUpListener() {
    ...
    let success = UIAlertController(title: "Confirm", message: "Registered Successfully", preferredStyle: .alert)
    let failure = UIAlertController(title: "Failure", message: "Registration Error", preferredStyle: .alert)
    let ok = UIAlertAction(title: "OK", style: .cancel)
    
    let newUser = RegisterModel(fullname: fullName, username: userName, email: email, password: password)
    AuthenticationAPI.instance.register(register: newUser) { (isSuccessful) in
      if isSuccessful {
